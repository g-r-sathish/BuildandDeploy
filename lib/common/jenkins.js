...
jenkinsService.postOrchestration = (orchestrationArray) => {
  ...
  if (config.jenkinsEnabled) {
    if (config._all.commit) {
      ...
      let params = {
        uri: `${config.jenkinsProtocol}://${config.jenkinsHost}:${config.jenkinsPort}/job/${config.jenkinsOrchestratedBuild}/build`,
        form: formData,
        auth: {
  ...
  let url = sprintf('https://%s/job/%s', config.jenkinsHost, config.jenkinsOrchestratedBuild);
